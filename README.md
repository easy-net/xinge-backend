...
- Virtual payment uses `WECHAT_VIRTUAL_OFFER_ID` (default `1450536598`), `WECHAT_VIRTUAL_APP_KEY`, `WECHAT_VIRTUAL_PRODUCT_ID`, and `WECHAT_VIRTUAL_ENV`. `goodsPrice` uses the order amount, which is validated against `product_config.current_amount` before payment params are created.
